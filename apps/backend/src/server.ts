  ...
  app.post(
    ...
    asyncHandler(async (request, response) => {
      ...
      const fundRows = funds.map((f) => {
        ...
        return `<tr><td style="padding:12px 16px;border-bottom:1px solid #f1f3f4;max-width:200px;"><div style="font-size:13px;font-weight:600;color:#202124;margin-bottom:2px;">${f.name}</div><div style="font-size:11px;color:#9aa0a6;font-family:monospace;">${f.code}</div></td><td style="padding:12px 16px;border-bottom:1px solid #f1f3f4;text-align:right;font-family:monospace;font-size:14px;font-weight:700;color:#202124;white-space:nowrap;">${f.nav}</td><td style="padding:12px 16px;border-bottom:1px solid #f1f3f4;text-align:center;white-space:nowrap;"><span style="display:inline-block;padding:3px 8px;border-radius:12px;font-size:12px;font-weight:700;background:${rB};color:${rC};">${f.zzl}</span></td><td style="padding:12px 16px;border-bottom:1px solid #f1f3f4;text-align:right;font-family:monospace;font-size:13px;font-weight:700;color:${pC};white-space:nowrap;">${f.dailyProfit != null ? `&yen;&nbsp;${f.dailyProfit}` : "&mdash;"}</td></tr>`;
      }).join("");
