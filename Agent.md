...
当需要根据资产截图或持仓快照（含有当前持有金额、持有收益率或持有收益）更新数据库中的持仓份额 (`holding_shares`) 与持仓成本价 (`cost_price`) 时，**必须且只能**采用以下公式进行计算：

1. **持仓份额计算**：
   $$\text{持仓份额} = \frac{\text{当前持有金额}}{\text{昨收单位净值 (dwjz)}}$$
   *注意：必须使用昨日收盘公布的单位净值（`dwjz`），绝对不能混入今日盘中的实时估算净值（`gsz`），因为天天基金等客户端在每日清算完毕前，持有金额均是按昨收净值折算的。*

2. **持仓成本价计算**：
   $$\text{持仓成本价} = \frac{\text{昨收单位净值 (dwjz)}}{1 + \text{持有收益率}}$$
   *其中：持有收益率（rate）使用截图中的百分比数值（如 -3.16% 则为 -0.0316）。这是解决红利再投、在途资金和历史偏差的最精确锚定方式。*
